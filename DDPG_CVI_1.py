...
class ActorNetwork(object):
    ...
    between -2 and 2
    """
    def __init__(self, sess, state_dim, action_dim, action_bound, learning_rate, tau, noise, beta,
    ...
    def cvi_update_mean(self, grad, var, sigma):
        ...
        update_mean = [var[i].assign(var[i] - self.learning_rate *
                                     self.beta * natural_grads[i]) for i in range(len(var))]
        return update_mean
...
def train(sess, env, actor, critic):
    ...
    for i in range(MAX_EPISODES):
        ...
        for j in range(MAX_EP_STEPS):
            ...
            #  openai didnot check the episode terminate
            if j == MAX_EP_STEPS - 1 :
                terminal = 1
            # if terminal:
            #     r += 100
        ...
        #                str(actor.noise) + '_lr_' + str(actor.learning_rate) + '_b_' + str(actor.beta) +  '.png'
            ...
            result_path = './cvi/lunar/cvi_n_' + \
                       str(actor.noise) + '_lr_' + str(actor.learning_rate) + '_b_' + str(actor.beta) +  '.npy'
            #result_path = './cvi/lunar/sgd_0.0001_noise_10000.npy'
            np.save(result_path, episode_reward)
...
def main(_):
    ...
    for beta in beta_list:
        for ACTOR_LEARNING_RATE in actor_lr_list:
            for NOISE in actor_noise:
